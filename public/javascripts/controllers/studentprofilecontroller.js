...
app.controller('studentProfileController', ['$http', '$scope','$rootScope', '$state', '$mdDialog', 'moment',
	function($http, $scope,$rootScope, $state, $mdDialog, moment ) {
	...
	$scope.updateStudent = function (student) {
		$http.put('/student/'+student._id,student).then(function (data) {
			if(student.oldpassword && student.newpassword && student.reenterpassword){
				...
				$http.put('/student/'+student._id+'/password',password).then(function (data) {
	...
	$scope.deleteStudent = function (ev,student) {
		...
		$mdDialog.show(confirm).then(function() {
			$http.delete('/student/'+student._id).then(function (data) {
				$rootScope.logout();
				$state.go('home');
				$mdDialog.show(alert);
			});
