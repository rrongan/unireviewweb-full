...
app.controller('collegeMainController', ['$http','$window','$rootScope','$scope','$state','$mdDialog','wikipediaFactory','$sce','moment',
	function($http, $window, $rootScope, $scope, $state, $mdDialog, wikipediaFactory, $sce, moment) {
		...
		$scope.updateReview = function (review) {
			$http.put('/college/'+ $state.params.collegeid +'/review/'+review._id,review).then(function () {
		...
		$scope.deleteReview = function (ev,review) {
			...
			$mdDialog.show(confirm).then(function() {
				$http.delete('/college/'+ $state.params.collegeid +'/review/'+review._id).then(function () {
					$mdDialog.show(alert);
					$state.reload();
				});
			}, function() {
		...
		$scope.addReview = function(){
			...
			if(reviewdetails.rating === 0){
				...
				$http.post('/college/'+$state.params.collegeid+'/review',reviewdetails).then(function () {
					var alert = $mdDialog.alert()
						.title('Review Added')
						.ariaLabel('Lucky day')
						.ok('Confirm');
